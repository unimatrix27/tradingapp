...
namespace trading.Background
{
        ...
        public class IbConnector
    {
        ...
        {  
            ew = new EWrapperImpl();
            clientSocket = ew.ClientSocket;
            clientSocket.AsyncEConnect = true;
            readerSignal = ew.Signal;
            ...
            new Thread(() => { 
                while(true){
                    if(clientSocket.IsConnected()){
                        ...
                        Console.WriteLine("Reconnecting TWs...");
                        Connect();
                    }
                }
        ...
        public void Connect()
        {
            ...
            //! [connect]
            clientSocket.eConnect("127.0.0.1", 4001, 0);
            //! [connect]

            //if(clientSocket.IsConnected()  == false) return;
            reader = new EReader(clientSocket, readerSignal);
            reader.Start();
            while (ew.NextOrderId <= 0) { }  
        ...
        public void test(){
            ...
            //clientSocket.reqHistoricalData(1234,contract,queryTime,"1 M","1 day","MIDPOINT",1,1,null);
            clientSocket.reqExecutions(9323,null);
        }
    }
}
